  ...
  const NavButton = ({ section, label, icon: Icon }) => {
    ...
    return (
      <button
        ...
        className={`
          w-full text-left ${isMobile ? "py-3 px-4" : "py-4 px-6"} rounded-2xl ${isMobile ? "mb-3" : "mb-4"} relative overflow-hidden group flex items-center ${isMobile ? "gap-3" : "gap-4"} backdrop-blur-sm
      ...
      >
        <div className={`
          ${isMobile ? "w-8 h-8" : "w-10 h-10"} rounded-xl flex items-center justify-center transition-all duration-300
          ${
            isActive
        ...
        <span className={`
          relative z-10 ${isMobile ? "text-sm" : "text-base"} font-medium transition-all duration-300
    ...
    <div className={`${
      ...
      <div className="relative z-10">
        ...
        <div className={`flex flex-col items-center ${isMobile ? "mb-6" : "mb-10"}`}>
          <div className={`relative group ${isMobile ? "mb-4" : "mb-6"} hover:scale-105 transition-transform duration-300`}>
          ...
          <h2 className={`${isMobile ? "text-xl" : "text-2xl"} font-bold mb-2 bg-gradient-to-r from-blue-400 to-purple-400 text-transparent bg-clip-text text-center`}>
            Parth Gajjar
          </h2>
          <div className="flex items-center gap-2 mb-3">
            <User className="h-4 w-4 text-gray-400" />
            <p className="text-sm text-gray-400 font-medium">Software Developer</p>
          </div>
          <div className={`px-3 py-1.5 bg-green-500/10 border border-green-500/20 rounded-full ${isMobile ? "text-center" : ""}`}>
            <span className="text-xs text-green-400 font-medium">Available for Co-op</span>
          </div>
        </div>
        ...
        {/* Social Links */}
        <div className={`${isMobile ? "space-y-3" : "space-y-4"} transition-all duration-300 ${
          activeSection === "hire" && !isMobile ? "scale-105" : ""
        }`}>
          {activeSection === "hire" && (
            <div className={`text-center ${isMobile ? "p-2" : "p-3"} bg-purple-500/10 border border-purple-500/20 rounded-2xl transition-all duration-300`}>
              <p className={`${isMobile ? "text-xs" : "text-xs"} text-purple-300 font-medium`}>
                Let's connect!
              </p>
            </div>
          )}
          
          <div className={`grid ${isMobile ? "grid-cols-4" : "grid-cols-2"} ${isMobile ? "gap-2" : "gap-3"} ${isMobile ? "p-3" : "p-4"} rounded-2xl transition-all duration-300 backdrop-blur-sm
            ${
              activeSection === "hire"
          ...
          >
            {socials.map((social, index) => (
              <a
                key={index}
                href={social.url}
                className={`${isMobile ? "p-2" : "p-3"} rounded-xl transition-all duration-300 flex items-center justify-center group backdrop-blur-sm ${!isMobile ? "hover:scale-110 hover:-translate-y-1" : "hover:scale-105"}
                  ${
                    activeSection === "hire"
                ...
                title={social.title}
              >
                <span className={`transition-all duration-300 ${isMobile ? "group-hover:scale-105" : "group-hover:scale-110"} ${isMobile ? "text-sm" : ""}`}>
                  {social.Icon}
                </span>
              </a>
            ))}
